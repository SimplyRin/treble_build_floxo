...
initRepos() {
    if [ ! -d .repo ]; then
        ...
        cp $BL/manifest.xml .repo/local_manifests/roomservice.xml
        echo
    fi
}
...
generatePackages() {
    ...
    xz -cv $BD/system-treble_arm64_bvN.img -T0 > $BD/floxo_arm64-ab-7.5-unofficial-$BUILD_DATE.img.xz
    xz -cv $BD/system-treble_arm64_bvN-vndklite.img -T0 > $BD/floxo_arm64-ab-vndklite-7.5-unofficial-$BUILD_DATE.img.xz
    rm -rf $BD/system-*.img
    echo
}
